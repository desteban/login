...
export async function emailBienvenida(persona: Persona, asunto: string) {
	try {
		let testAccount = await nodemailer.createTestAccount();
			...
			transporter.sendMail({
				...
				>
					<p>
						...
						<a href="${process.env.PAGEUTL}/${persona.token}" target="_blanck">enlace</a> para activar y verificar tu cuenta o puedes
						copiar el siguiente link ${process.env.PAGEUTL}/${persona.token}
					</p>
				</div>
